...
from src.schemas.rooms import RoomAddDto, Room, RoomAdd, RoomPatchDto, RoomPatch
...
class RoomService(BaseService):
    ...
    async def create_room(self, hotel_id: int, room_data: RoomAddDto):
        try:
    ...
    async def edit_room_partially(self, room_id: int, hotel_id: int, room_data: RoomPatchDto):
        await HotelService(self.db).get_hotel_with_check(hotel_id)
        await self.get_room_with_check(room_id)
        ...
        if "facilities_ids" in _room_data_dict:
            await self.db.rooms_facilities.set_facilities(
                facilities_ids=_room_data_dict["facilities_ids"], room_id=room_id
            )
        await self.db.commit()
